...
import { createSabiProvider, SabiLanguageModel } from "./ai-sdk";
import { createSabi } from "./index";
import { SabiError } from "./errors";
...
describe("createSabiProvider", () => {
  ...
  it("languageModel returns a SabiLanguageModel", () => {
    const sabi = createSabi({ groq: { apiKey: "gsk_abc" } });
    const provider = createSabiProvider(sabi);
    const model = provider.languageModel("groq/llama-3.1-8b-instant");

    expect(model).toBeInstanceOf(SabiLanguageModel);
...
describe("SabiLanguageModel.doGenerate", () => {
  it("returns text content", async () => {
    setFetch(async () =>
      okResponse({
        choices: [{ message: { content: "Hello from Sabi!" } }],
        usage: { prompt_tokens: 5, completion_tokens: 10, total_tokens: 15 },
      })
    );

    const sabi = createSabi({ groq: { apiKey: "gsk_abc" } });
    const model = new SabiLanguageModel(sabi, "groq/llama-3.1-8b-instant");
  ...
  it("passes generation parameters to Sabi", async () => {
...
describe("SabiLanguageModel.doStream", () => {
  ...
  it("converts messages to Sabi format for stream", async () => {
    let sentBody: unknown;
    setFetch(async (_url, init) => {
      sentBody = JSON.parse(init?.body as string);
      return sseResponse(

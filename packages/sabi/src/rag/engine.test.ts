...
describe("RagEngine", () => {
  ...
  it("loadStream loads content that is then queryable", async () => {
    const { engine, cleanup } = makeEngine();
    const events: string[] = [];
    for await (const event of engine.loadStream({
      name: "stream-query.md",
      content: "The quick brown fox jumps over the lazy dog near the riverbank.",
    })) {
      events.push(event.type);
    }
    expect(events).toContain("file_done");
    expect(events).toContain("done");
    expect(engine.stats().files).toBe(1);
    const results = await engine.query("fox riverbank");
    expect(results.length).toBeGreaterThan(0);
    expect(results[0]!.content).toContain("fox");
    cleanup();
  });

  it("persists content across close and reopen", async () => {
    const path = `.sabi/test-rag-persistence-${Date.now()}.db`;
    ...
    expect(engine2.stats().files).toBe(1);
    const results = await engine2.query("persistence");
    expect(results.length).toBeGreaterThan(0);
  ...
  it("deduplicates across close and reopen", async () => {
    const path = `.sabi/test-rag-dedup-${Date.now()}.db`;
    const engine1 = new RagEngine({
      dbPath: path,
      embeddingModel: "test/text-embedding",
      chunkSize: 100,
    ...
    engine1.setProviders({ provider: "openai", apiKey: "test-key" }, {});
    await engine1.load({ name: "dedup.md", content: "Deduplication test content." });
    engine1.close();

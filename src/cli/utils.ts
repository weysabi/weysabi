...
export function loadConfig(): { config: SabiConfig; path: string } | null {
  const path = findConfigPath();
  if (!path) return null;
  try {
    const raw = readFileSync(path, "utf-8");
    const config = JSON.parse(raw) as SabiConfig;
    return { config, path };
  } catch {
    return null;
  }
...
export function saveConfig(config: SabiConfig, path?: string): string {

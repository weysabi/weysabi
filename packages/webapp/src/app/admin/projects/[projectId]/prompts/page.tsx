...
import { MessageSquare, Plus, ExternalLink, Eye, Search } from "lucide-react";
...
export default function PromptsPage() {
  ...
  async function loadPrompts() {
    setLoading(true);
    try {
      const url = buildListUrl(`/v1/projects/${projectId}/prompts`, page, limit, { search });
      const res = await apiFetch(url);
      if (res.ok) {
        const data = (await res.json()) as { items: Prompt[]; total: number };

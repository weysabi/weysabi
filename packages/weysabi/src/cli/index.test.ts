...
describe("loadConfig / saveConfig", () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync("weysabi-test-");
  });
  ...
  it("saves and loads config", () => {
    const configPath = resolve(tmpDir, "weysabi.json");
    const config = {
...
describe("ensureGitignore", () => {
  it("creates a gitignore that excludes local config and data", () => {
    const tmpDir = mkdtempSync("weysabi-gitignore-");
    try {
      ensureGitignore(tmpDir);
      const content = readFileSync(resolve(tmpDir, ".gitignore"), "utf-8");
      expect(content).toContain(".env");
      expect(content).toContain(".weysabi/");
      expect(content).toContain("weysabi.json");
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it("preserves existing entries without adding duplicates", () => {
    ...
    try {
      ...
      expect(content.match(/\.weysabi\//gu)).toHaveLength(1);
      expect(content.match(/weysabi\.json/gu)).toHaveLength(1);
...
describe("upsertEnvFile", () => {
  it("writes new credentials and updates matching keys without replacing other values", () => {
    const tmpDir = mkdtempSync("weysabi-env-");
    try {
...
describe("create command", () => {
  ...
  it("creates a server starter without installing dependencies", async () => {
    const tmpDir = mkdtempSync("weysabi-create-");
    try {
      ...
      expect(server).toContain("WEYSABI_ADMIN_API_KEY");

      const env = readFileSync(resolve(projectDir, ".env.example"), "utf-8");
      expect(env).toContain("OPENAI_API_KEY=");
      expect(env).toContain("WEYSABI_CONTROL_DB=.weysabi/control.db");
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
    }
  });
